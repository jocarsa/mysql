                ...
                $cadena .= "<form method='POST' action='?tabla=".$_GET['tabla']."'>";
                        while($fila = mysqli_fetch_assoc($resultado)){
                             if(strpos($fila['Field'], "_") !== false){
                                 $cadena .= "<td><select name='".$fila['Field']."'>";
                                 $cadena .= "<option>Selecciona una opción</option>";
                                 $peticion2 = "SELECT * FROM ".explode("_",$fila['Field'])[0]."s"."";
                                $resultado2 = mysqli_query($conexion,$peticion2);
                                 while($fila2 = mysqli_fetch_assoc($resultado2)){
                                     $cadena .= "<option value='".$fila2['Identificador']."'>";
                                     foreach($fila2 as $micolumna){
                                         $cadena .= $micolumna." - ";
                                     }
